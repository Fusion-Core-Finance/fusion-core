...
- **PROD-FN** — a mutation inside a production function in the rule's cone (the shared
  `supply_transition` fns, the `bucket` helpers, `recovery::absorb`, `fusd_oracle::aggregate`).
  Flips the Certora rule AND the runnable suite — the highest assurance class.
- **HANDLER** — a mutation in handler-only code or at the shared-transition call site (dropping the
  call, or the assignment of the returned post-state). Flips ONLY the runnable suite; every Certora
  rule stays green by construction — the documented residual gap.
