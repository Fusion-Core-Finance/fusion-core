...
Thresholds are stored as the smallest sufficient int (bps fit `u16`; the clamps guarantee it), with a 62-byte `_reserved` for forward-compat (e.g. a second Pyth feed for LST redemption-rate pricing).
