...
FUSD is an **overcollateralized CDP stablecoin** in the Liquity lineage, built natively for Solana with Anchor, administered through bounded governance confined to hard, code-enforced limits. A user locks approved collateral in a *position* (a Maker "vault" / Liquity "trove") and mints FUSD against it; the protocol holds no custody, runs no off-chain key, and exposes no privileged lever over user funds. It is single-mint (one FUSD token), multi-collateral, and isolated per collateral.
...
- **Bounded governance.** Governance may only tune risk parameters inside compile-time `[min,max]` clamps, may mark only registry/config accounts writable, and can never seize, freeze, mint outside the rules, or retroactively change existing positions' terms. The gate never assumes how the upstream authority makes its decisions — any authorized signer or PDA can serve. The locks are structural, not procedural.
...
| **Interest rates** | **User-set** (Liquity v2 / BOLD), debt-in-front | Each borrower picks their rate; redemptions hit lowest-rate debt first. Governance sets only the min/max bounds, never an individual's rate. |
...
| `migrate_gov_authority` / `accept_gov_authority` | [Built] | Two-step propose/accept rotation of the bootstrap/admin `ProtocolConfig.gov_authority` (the key later rotated to a successor authority as governance minimizes — Phase 3). |
...
| `guardian` | `Pubkey` | De-risk-only role, independent of the governance authority, so a frozen or unresponsive governance system cannot also freeze FUSD's emergency response. [Built] — consumed by `guardian_derisk` (per-market borrow pause); rotatable/revocable by `gov_authority` via `set_guardian`. |
...
|---|---|---|
| `init_reactor_pool` | `gov_authority` | Creates the `ReactorPool`, the zero-copy grid (zero-initialized via `load_init`), and both vaults for a market. `p = DECIMAL_PRECISION`, `epoch = scale = 0`. One per market. Sets a `Market.liq_infra_flags` readiness bit (the L-02 borrow gate). |
...
Liquidation absorbs the debt through a **five-tier waterfall** (`fusd_math::recovery::absorb`, conservation `reactor + redist + buffer + global + unhomed == debt` Kani-proven): **(1)** Reactor Pool offset, **(2)** redistribution to other positions, **(3)** the per-market **insurance buffer** — a pre-funded `InsuranceBuffer` PDA whose FUSD vault is funded by `fund_buffer` + the lazy interest mint (§2.2) — burns what's left, **(3.5)** the shared **global backstop reserve** (bounded second-loss capital, drawn up to a per-market hybrid cap when the local buffer is exhausted), **(4)** any remainder that still has no home is booked as `Market.bad_debt`, its offsetting collateral retained as `Market.protocol_collateral`, and the market is tripped into `shutdown` (`SHUTDOWN_REASON_UNHOMED_BAD_DEBT`). So liquidation never stalls and never silently wraps an accumulator. The un-homed loss is later recovered through the value-recovery trio (`sweep_protocol_collateral` → off-chain sale → `settle_bad_debt` burns recovered FUSD against `bad_debt`) and, automatically, through the **C16 auto-paydown interlock**: while `bad_debt > 0`, `refresh_market` diverts a governable (default-off) fraction (`bad_debt_paydown_bps`) of the interest it would otherwise mint to the buffer and uses it to retire `bad_debt` instead — a self-limiting recapitalization-from-revenue that amortizes the loss out of ongoing interest without a governance round-trip (supply-preserving: the diverted slice is simply not minted while `bad_debt` drops by the same amount). The buffer **funding-source policy** (target size, fee share, exhaustion) is the only [Planned] remainder.
...
6. **Redeem each.** Redemption pays at the **MID** oracle price `mid = (spot + debt_spot)/2` — the un-skewed price, NOT the conservative LOW `spot`: dividing the outflow by the low price hands the redeemer more collateral than a dollar is worth, so a wide confidence band would make redemption profitable at peg and drain the lowest-rate borrower (BOLD/Liquity pay at the single oracle price). For each candidate until `amount` is exhausted: present `debt = recorded_debt` (post-realize) and `coll_value = ray_mul(ink, mid)`, then `redeem_amt = min(remaining, debt, coll_value)`. The `coll_value` cap means redemption **never creates bad debt** on an underwater position. Collateral removed is `mul_div_floor(redeem_amt, RAY, mid)` (floored, capped at `ink`); the position always loses exactly that. **After the loop**, the base-rate is bumped with THIS redemption's volume and the fee is charged at the **post-bump** rate — BOLD-faithful: Liquity updates the base-rate before `_getRedemptionFee`, so a large redemption pays the deterrent its own size triggers (the old pre-bump ordering let the first redemption in a window escape with just the flat floor). `fee_coll = total_coll_drawn · fee_bps / 10_000` is retained in `Market.surplus_collateral`; the redeemer receives the rest. `fee_bps` is the flat `redemption_fee_bps` UNLESS the C9 dynamic base-rate is enabled (`redemption_base_rate_max_bps > 0`), in which case it is `clamp(redemption_fee_bps + min(base_rate_bps, redemption_base_rate_max_bps), floor, MAX_REDEMPTION_FEE_BPS)` on `Market.redemption_base_rate` — decayed to now at a 6h half-life, then bumped by `(redeemed / market debt) / BETA` **before** the fee is computed (`fusd_math::redemption`). `urgent_redeem` is always 0-fee, also pays at mid, and never touches the base-rate. `recorded_debt`/`ink`, `agg_recorded_debt`/`total_collateral`, and the stake are updated; a position drained to `ink == 0` with residual debt moves to the zombie pen.
...
| Fuzz + Certora **[Built]** | B8 proptest (bitmap `BTreeSet` model + `cmp_collateral_ratio` order laws) plus the Certora #2a bitmap-coupling invariant (`words ⟺ counts`; single-touch add/remove helpers at a concrete representative bucket), VERIFIED on the cloud and mutation-checked |
...
`TwapConfig` (`min_samples`, `max_staleness`) lives in per-collateral params, not in the ring account itself, so it stays governance-tunable within compile-time clamps.
...
Thresholds are stored as the smallest sufficient int (bps fit `u16`; the clamps guarantee it). `MarketOracle` also binds **`lst_stake_pool`** — the stake-pool account for the C1 canonical-rate leg (`Pubkey::default()` = a non-LST market, leg off); init requires a 9-decimal collateral mint when it is set. Three further carves serve the canonical-primary mode (§6.1): **`canonical_primary: u8`** (the mode flag; `0` on every pre-carve account = market-feed oracle, bit-identical prior behavior), **`liquidity_haircut_bps: u16`** (mandatory `[1, 2_000]` in mode 1, must be `0` otherwise), and **`last_canonical_rate_ray: u128`** (the last committed pool rate; a decrease arms the negative-NAV liquidation grace — FUSOL-05). The first two are init-only; the rate is written by `update_price` on each committed mode-1 crank. The remaining `_reserved` is 11 bytes (was 62; `lst_stake_pool` carved 32, the mode fields 3, the rate 16) for forward-compat.
...
A crucial accuracy note up front: **the bounded `GovernanceGate` + FUSD-owned timelock are now [Built]** and govern market-parameter changes through a two-speed flow (`queue_param_change` → delay → permissionless `execute_param_change`), with compile-time clamps enforced at both queue and execute. The decision-#2 localnet PoC that gated this work is **done and passing** (an external program-derived signer can queue a change through the gate). The de-risk **`guardian_derisk` + `set_guardian`** and the terminal **`shutdown` + `urgent_redeem`** are now [Built] (§7.2, §8). The inbound-authority handoff is **two-step** (propose → the successor signs to accept), and every executed change re-checks the per-field clamps **plus** the relational `validate_market_config` bounds (collar fundability, RP-solvency, `mcr ≥ scr`) and emits a forensic `prev_value`→`value` trail; an MCR raise additionally arms the liquidation grace window. The gate applies **twenty-one `MarketParam`s** — the thirteen `Market` knobs (`Mcr`, `DebtCeiling`, `RedemptionFee`, `LiqGasComp`, `RateLimitCap`, `Ccr`, `LiqBonus`, `MinDebt`, `RateAdjustCooldown`, `KeeperReward`, `BorrowFee`, `BadDebtPaydown`, `RedemptionBaseRateMax`) plus the **RiskParamRegistry** set (`OracleMaxConf`, `OracleMaxDeviation`, `OracleTwapDivergence`, `OracleLiqDivergence`, `OracleMaxAge`, `OracleK`, `OracleTwapStaleness` on the `MarketOracle`, threaded through queue/execute via an optional oracle account; plus `Scr` on the `Market`) — all behind the same timelocked, doubly-clamped flow with the relational guards (`mcr ≥ scr`, oracle `liq ≥ twap`). So the RiskParamRegistry is realized as an extension of the existing `MarketParam` set, not a separate `[b"registry"]` account. A few structural oracle params stay **init-time-only** by design (`twap_window_secs`, `twap_min_samples`, the plausibility band, the reserve bond, and `bucket_width`). `config.gov_authority` is the bootstrap/admin authority (creates markets, oracles, SPs, and the gate; itself rotated via the two-step `migrate_gov_authority`/`accept_gov_authority`); the gate's own **migratable `inbound_authority`** is the param-tuning authority that queues timelocked changes.

### 7.1 The bounded governance authority model [Built]

FUSD's governance is **implementation-agnostic by construction**. The protocol defines *what* governance may change (a clamped parameter set), *how* a change lands (a protocol-owned timelock with a permissionless execute), and *who* may queue one (a single migratable inbound authority) — and deliberately defines nothing about how that authority makes its decisions. The `GovernanceGate` accepts an authorized signer or PDA via `inbound_authority`; whether the key upstream is a hardware wallet, a multisig, a DAO's execution PDA, a timelock contract, or a token-voting system is invisible to the gate. No external governance framework is required, assumed, or preferred.

The parameter-change flow:

```
external governance (any authorized signer or PDA — the migratable inbound authority)
        → GovernanceGate: queue_param_change   (authority check + compile-time clamps, fail-fast)
        → wait timelock_secs                   (protocol-owned delay; no fast path)
        → execute_param_change                 (permissionless; clamps + relational bounds re-checked)
```

#### The `GovernanceGate` and the migratable inbound authority [Built]

The FUSD-owned **`GovernanceGate` PDA** (`[b"gov_gate"]`) is the **sole** authority on the market-parameter setter. It stores a *single migratable inbound authority* (`inbound_authority`) that every `queue_param_change` `require_keys_eq!`s against, plus the `timelock_secs` delay and a `queue_nonce`. That one indirection is the whole point: the **two-step** `migrate_inbound_authority`/`accept_inbound_authority` handoff (propose → the successor signs to accept) repoints it from the guarded-launch authority to any successor — a different multisig, a governance program's execution PDA, any compatible signer — without redeploying or touching any other code path. The decision-#2 localnet PoC proved an external program-derived signer can queue a change through the gate; the two-step handoff itself is covered by the litesvm governance suite (predecessor proposes, successor signs to accept).

Separately, `ProtocolConfig.gov_authority` (a `Pubkey`) is the bootstrap/admin authority: every init-time governance instruction (`init_market`, `init_market_oracle`, `init_reactor_pool`, and the dev-only `dev_set_price`) gates on it via `require_keys_eq!(authority, config.gov_authority)`, and it rotates via its own two-step `migrate_gov_authority`/`accept_gov_authority`. It is distinct from the gate's `inbound_authority`, the param-tuning key.

**Honest framing:** the gate validates *authority + clamps + timelock*; it cannot validate the integrity of whatever decision process sits upstream. Placing an external governance system behind the inbound authority adds that system's own trust surface (its upgrade authority, its execution semantics), which an integrator verifies directly — by pinning a release tag and checking the on-chain program hash (`solana-verify get-program-hash`), never by trusting an audit-commit label. The gate buys *migratability and confinement*, not a claim that whatever sits upstream is trustless.

#### FUSD owns its own timelock [Built]

FUSD assumes **no execution delay upstream**: an inbound authority may be able to execute the instant its own decision process concludes. FUSD therefore supplies its own. Each queued change is a **`TimelockedParam`** PDA (`[b"timelock", nonce_le]`, one per op) carrying its `eta = now + timelock_secs`, target market, param, and value. The setters split as **`queue_param_change`** (inbound authority; clamps fail-fast) → (wait `timelock_secs`) → **`execute_param_change`** (permissionless once `now ≥ eta`; re-validates clamps, applies, closes the op), with **`cancel_param_change`** to withdraw a queued op. This is the *slow lane only* — there is **no immediate in-gate fast-path** (a misclassification there is a governance-bypass, the Beanstalk class), so every param change pays the delay, giving users an exit window before it binds. The delay is itself a bounded gov param (`[MIN, MAX]_GOV_TIMELOCK_SECS`, 0–30 days). Surplus/insurance outflow changes are designed to ride the same slow lane with a per-epoch hard cap (still [Planned]).

### 7.2 The de-risk-only guardian [Built]

Emergencies route through a **guardian role that is independent of the governance authority** — so a frozen or buggy upstream governance system cannot also freeze FUSD's emergency response. `ProtocolConfig.guardian: Pubkey` is set at `init_protocol` and consumed by **`guardian_derisk` [Built]**, which is **monotonic de-risk only**. The envelope is constitutional:
...
Three structural properties, not policies, are what reduce the governance authority to a bounded tuner:
...
2. **Account-level confinement [Built/structural].** Governance instructions may mark **only** parameter accounts (`Market` param fields / `MarketOracle`) writable — never a `Position`/`Market` *balance* field, the mint, or any escrow vault. "No seize/freeze/mint-outside-rules" is enforced by which accounts the instruction is even allowed to declare mutable.
...
What governance may do: initialize protocol components; onboard collateral markets; configure supported oracle infrastructure (program IDs, feed IDs, thresholds within clamps); rotate the de-risk guardian; queue permitted parameter changes through the `GovernanceGate` (debt ceilings + auto-line gap/ttl, MCR/CCR/SCR, fees and incentives, interest-rate min/max bounds — all within clamps); perform the narrowly defined protocol-owned value-recovery operations. What it may **never** do: freeze FUSD balances; seize user collateral; mint FUSD outside the borrowing/interest-accounting rules; bypass the compile-time parameter clamps; disable permissionless repayment or liquidation; disable the applicable redemption or shutdown exit path; directly rewrite user positions; add a freeze authority to the FUSD mint. (The BPF upgrade authority remains the one lever above these locks until the Phase-4 lockdown — §10.5.)
...
**Twenty-one `MarketParam`s are gate-tunable** (timelocked `queue_param_change` → `execute_param_change`, clamps re-checked at both, plus the relational `validate_market_config` + oracle `liq ≥ twap` bounds): the thirteen `Market` knobs `Mcr`, `DebtCeiling`, `RedemptionFee`, `LiqGasComp`, `RateLimitCap`, `Ccr`, `LiqBonus`, `MinDebt`, `RateAdjustCooldown`, `KeeperReward`, `BorrowFee`, `BadDebtPaydown`, `RedemptionBaseRateMax`, plus the **RiskParamRegistry** set `OracleMaxConf`, `OracleMaxDeviation`, `OracleTwapDivergence`, `OracleLiqDivergence`, `OracleMaxAge`, `OracleK`, `OracleTwapStaleness` (on `MarketOracle`, via an optional oracle account threaded through queue/execute) and `Scr` (on `Market`). The reserve bond, rate-bucket width, the TWAP window/min-samples, and the plausibility band stay **init-time-only** (structural / deliberately non-retroactive — the bond is fixed per-position at open and `Position.bucket` is stored).
...
#### Oracle parameters (set at `init_market_oracle`, per-market `MarketOracle`) [Built — gate-tunable via the RiskParamRegistry `MarketParam`s, except the structural window/min-samples/band which stay init-time-only]
...
|---|---|---|
| SCR (shutdown ratio) | per-market `Market.scr_bps` / `MarketParam::Scr` | [Built] — gate-tunable (clamp `[MIN_SCR_BPS, MAX_SCR_BPS]`, relational `≤ mcr`); drives the `shutdown` TCR-breach trigger. |
...
The strong global invariant `FUSD minted == Σ market debt` **cannot** be a single hot counter without serializing every `borrow` behind one write-lock (the failure mode that killed Maker's global `Vat` on Solana). So it is kept as **sharded per-market debt counters** (`Market.agg_recorded_debt` per collateral) and re-derived by a periodic **permissionless reconciliation crank**, `reconcile_supply` **[Built]**: it sums `Σ_market (agg_recorded_debt − unminted_interest + bad_debt)` over the submitted markets (computed underflow-safe as `Σ(agg + bad) − Σ unminted`, each market program-owned + deduped), compares it to the live mint supply, and stamps `{mint_supply, backing, residual}` on the `SupplyReconciliation` singleton + a `SupplyReconciled` event. `residual == 0` ⇒ reconciled; non-zero ⇒ an off-chain alarm (an accounting drift, or a market omitted from the crank's input). The always-enforced per-vault hard solvency check (above) guarantees individual solvency; the reconciliation crank is an *auditability* layer that **gates no user path**. (A single pass covers every launch-era market; a sharded multi-tx accumulation is the documented extension past `MAX_RECONCILE_MARKETS`.)
...
- **Governance.** Hubble is a conventional HBB token DAO that can change fees (it raised its stability fee 10%→12.5% in March 2024 to defend the peg). FUSD uses **bounded, implementation-agnostic governance** — every parameter change rides a compile-time-clamped, protocol-owned timelock, bounded by construction so governance can never seize/freeze/mint (see §7).
...
- **Bounded governance as the only parameter path** — every governable parameter of the stablecoin rides one compile-time-clamped, protocol-owned timelock behind a single migratable inbound authority, with no in-gate fast lane; no surveyed live Solana stablecoin confines its governance this way (see §7.1).
...
- **Proof-of-reserves (supply-reconciliation crank)** — the on-chain crank is **[Built]** (`reconcile_supply` re-derives `Σ market backing` vs the mint supply, stamping a queryable residual). The **third-party audit + formal-verification** half remains `[Planned]` (only internal adversarial review + the Kani/Certora work exists today). Non-negotiable before public launch.
...
- **Using the collateral allowlist or a governance action to target a specific actor's deposit.** De-listing a blue-chip asset to stop one actor is collateral-blunt and ineffective (they use an already-listed liquid asset); targeting an individual is a binary censorship decision governance is structurally the wrong tool for, and a liability magnet. The allowlist is for **asset-class curation, never for targeting individuals.**
...
| `init_reactor_pool` | governance | Create the RP: FUSD vault, seized-collateral vault, bounded `EpochToScaleToSum` grid; sets a `Market.liq_infra_flags` bit (the L-02 borrow gate). | [Built] |
| `init_insurance_buffer` / `fund_buffer` | governance / anyone | Create the per-market fUSD insurance buffer (sets a `Market.liq_infra_flags` bit — the L-02 borrow gate) / permissionlessly fund it. | [Built] |
...
|---|---|---|---|
| ~~`RiskParamRegistry`~~ (realized in `MarketParam`) | — | The broader param set (oracle thresholds + `scr_bps`) is now gate-tunable as **eight added `MarketParam`s** threaded through the existing timelock (the `MarketOracle` is an optional account on queue/execute) — no separate `[b"registry"]` account is needed. The `[b"registry"]` seed stays reserved. | [Built] |
...
| `SupplyReconciliation` | `[b"supply_recon"]` | Proof-of-reserves singleton: the permissionless `reconcile_supply` crank stamps `{last_ts, market_count, mint_supply, backing, residual}` here. Auditability only; gates nothing. | [Built] |
...
| Phase | Scope | Status |
|---|---|---|
| **Phase 1 — Guarded launch / core** | CDP flow + per-position BOLD interest + Reactor Pool + the 5-tier liquidation waterfall with SOL-bond + gas-comp incentives + the rate-bucket redemption floor, on 1–2 collaterals; authority = a guarded launch authority (a designated signer or PDA — e.g. a security-focused multisig, as an implementation choice) behind the gate's own timelock. | **Core mechanisms [Built]** and litesvm-tested + adversarially reviewed; the **`GovernanceGate` + timelock**, the **live oracle cranks**, the **de-risk guardian**, the terminal **`shutdown`/`urgent_redeem`** breaker, the net-outflow **rate limiter**, the **CCR band**, the **on-resume grace**, the **insurance buffer + value-recovery**, per-position interest, the klend-review hardening (vault assert, config bounds, MCR-grace, event-CPI, two-step authorities), and the **B8 fuzz pass** are all **[Built]**. The oracle layout-freeze window (B3 divergence gate, B5 `debt_price`, C6 plausibility band, D3 Pyth-migration-updatable IDs) and the first **Certora core-invariant pass** have since landed (bitmap-helper coupling and the absorb waterfall cloud-VERIFIED against production functions; the supply family proven over the shared supply-transition fns — all eight rules cloud-VERIFIED with every S1–S8 production-mutation flip confirmed — and the C1 oracle-cap pair cloud-VERIFIED; handler call-site coverage remains litesvm-only); the remaining launch-gating work is external **audits** (≥2), the **bitmap concurrent-flip Certora** pass, a **verified build**, and parameter calibration — so the milestone is **functionally complete, not launch-complete**. |
...
| **Phase 3 — Governance-minimized** | Broader parameter confinement and a reduced authority surface; rotate the gate's inbound authority to a successor authority (any compatible signer or PDA) via the built two-step handoff, behind FUSD's own timelock; every param has enforced compile-time clamps. | [Planned] |
...
- **Governance:** the `GovernanceGate` + FUSD-owned timelock and their setters are **[Built]** — the **twenty-one `MarketParam`s** (the thirteen `Market` knobs + the eight-strong RiskParamRegistry set: oracle thresholds + `scr_bps`) are governance-tunable behind the clamped delay (with the relational `validate_market_config` + oracle `liq ≥ twap` bounds + the forensic `prev_value` trail; two-step authority handoffs; decision-#2 PoC passing). The independent **`guardian_derisk` + `set_guardian` are [Built]** (§7.2). Remaining init-time-only by design: `reserve_lamports`, `bucket_width_bps`, the TWAP window/min-samples, and the plausibility band.
...
- **Other:** `CollSurplus` + `claim_coll_surplus`; the surplus/insurance buffer; deferred CDP litesvm cases (debt-ceiling 6006). (The global supply-invariant reconciliation crank is now [Built] — `reconcile_supply`.) Before launch, the **novel-on-Solana** redemption bitmap still owes bespoke fuzz + Certora — especially the concurrent "last member leaves a bucket" flip, currently serialized only by the per-market `Market` write-lock.
...
rebalances under a 3% global churn budget in an epoch-rotated, caller-unsteerable two-pass
cursor order (drains first, then deficit/surplus moves walked from a rotating start index).
The cursor order is a **recorded deviation** from the subsystem spec's per-epoch global
greatest-deficit-first priority — a global maximum is not verifiable on-chain at 1024
validators without caller-steerable subset selection; it is a deterministic, batchable,
non-steerable approximation of that intent, and amending the spec text to match is an open
item pending economic review (see [`docs/stake-pool/README.md`](stake-pool/README.md)).
...
| Market params (the twenty-one `MarketParam`s) | Governance | [Built] | the thirteen `Market` knobs + the eight RiskParamRegistry params (oracle thresholds + `Scr`) gate-tunable via timelock; reserve/bucket-width/TWAP-window/band init-time-only (non-retroactive) |
...
| Oracle params (conf, deviation, TWAP corridor, age, k, staleness) | Governance | [Built] | gate-tunable via the RiskParamRegistry `MarketParam`s (timelocked, clamped, oracle account threaded through); the TWAP window/min-samples + plausibility band stay init-time-only; per-market `MarketOracle`; defaults are placeholders |
...
| FUSD-owned timelock (`TimelockedParam`) | Governance | [Built] | assumes no delay upstream of the gate; FUSD enforces its own (queue→eta→execute) |
| RiskParamRegistry (broader param set) | Governance | [Built] | realized as eight added `MarketParam`s (oracle thresholds + `scr_bps`) on the existing timelock + an optional `MarketOracle` account; no separate `[b"registry"]` account |
...
| SCR (shutdown ratio) | Governance | [Built] | `Market.scr_bps` / `MarketParam::Scr` — gate-tunable (clamp `[MIN_SCR_BPS, MAX_SCR_BPS]`, relational `≤ mcr`) |
...
| Governance bounded / non-retroactive / can't mint-move-freeze-seize | Security | [Built] | `GovernanceGate` + timelock + `guardian_derisk`/`set_guardian`; the twenty-one `MarketParam`s (incl. the RiskParamRegistry oracle/scr set) clamped + relationally validated at queue+execute |
...
| Global-supply auditability (sharded counters + reconciliation crank) | Security | [Built] | Per-market `agg_recorded_debt` shards + the permissionless `reconcile_supply` crank (residual stamped on `SupplyReconciliation` + event) |
