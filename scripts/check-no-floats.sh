...
done < <(find programs/fusd-core/src programs/fusion-stake-controller/src crates/fusd-math/src crates/fusd-oracle/src crates/fusion-stake-math/src crates/fusion-stake-view/src -name '*.rs' -print0)

if [ "$fail" -ne 0 ]; then
  echo "No floats in the SBF money path — use integer WAD/RAY fixed-point + bnum::U256." >&2
  exit 1
fi
echo "OK: production SBF path (fusd-core / fusion-stake-controller / fusd-math / fusd-oracle / fusion-stake-*) is float-free."
